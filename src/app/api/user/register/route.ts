...
export async function POST(
    ...
    try {
        ...
        if (user && user.length > 0) {
            return result.error(403, await getTranslation(req, 'api.errors.systemInitialized', 'System has been initialized, registration is not allowed. Please contact the administrator.'));
        }
